document.addEventListener('DOMContentLoaded', function () {
  ...
  let selectedCategory = 'WOMENS'; // Переменная для хранения выбранной категории
  ...
  function addSubmenuEventListeners() {
    ...
    submenuLinks.forEach(link => {
      link.addEventListener('click', function (event) {
        ...
        // СНЕСТИ ОТСЮДА НАХУЙ .HTML В КОНЦЕ ПРИ ДЕПЛОЕ
        const subCategoryURL = `/shopping/${selectedCategory.toLowerCase().replace(/\s+/g, '-')}-${this.textContent.toLowerCase().replace(/\s+/g, '-')}.html`;
  ...
  mainMenuLinks.forEach(menuItem => {
    menuItem.addEventListener('click', function (event) {
      ...
      selectedCategory = this.textContent; // Сохраняем выбранную категорию
